...
namespace Fractural.Utils
{
    ...
    public static class NodeUtils
    {
        ...
        public static int GetChildIdx(this Node node, Node child)
        {
            if (child.GetParent() != node)
                GD.PushError($"{nameof(GetChildIdx)}: Expect child to be a child of node!");
            return node.GetChildren().IndexOf(child);
        }

        public static void SwapChildren(this Node node, Node child, Node otherChild)
        {
            if (child.GetParent() != node || otherChild.GetParent() != node)
            {
                GD.PushError($"{nameof(SwapChildren)}: Cannot swap children that don't have the same parent!");
                return;
            }
            int childIdx = node.GetChildIdx(child);
            int otherChildIdx = node.GetChildIdx(otherChild);
            node.MoveChild(otherChild, childIdx);
            node.MoveChild(child, otherChildIdx);
        }
    }
}
